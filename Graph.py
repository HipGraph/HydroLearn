...
def test():
    ...
    comm_var.set("data_source", data_src)
    dist_var = var.get("distributed")
    src_var = var.get(data_src)
    spatmp_var = Container().copy([src_var, comm_var, dist_var])
    spatmp = SpatiotemporalData(spatmp_var)
    grph_var = Container().copy([comm_var, var.get("graph")])
    grph_var.set("construction_method", "prodige")
    G = Graph(spatmp, grph_var)


if len(sys.argv) > 1 and sys.argv[0] == "Graph.py" and sys.argv[1] == "test":
    test()
